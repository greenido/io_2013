...
if ( !$url ) {
  ...
  if ( strtolower($_SERVER['REQUEST_METHOD']) == 'post' ) {
    ...
    curl_setopt( $ch, CURLOPT_POSTFIELDS, $postData); // TODO - change !!! -->  $_POST );
    error_log("==== working on POST: ". $postData );
    print_r($_POST);
    error_log("sadsd############################:" . $_SERVER['REQUEST_METHOD'] . "\n");
    var_dump($_REQUEST);
    error_log("ssdsdsdsds\n");
  }
//  
...
//  
  ...
  curl_setopt( $ch, CURLOPT_USERAGENT,  $_SERVER['HTTP_USER_AGENT'] ); //$_GET['user_agent'] ? $_GET['user_agent'] :
  $res = curl_exec( $ch );
  if ($res === FALSE) {
    die(curl_error($ch));
  ...
  print_r($ch);
  error_log("^2222222222^^" . $res . ' user agent: ' .$_SERVER['HTTP_USER_AGENT']);
...
if (isset($_GET['url']) ) {
  ...
  error_log($_GET);
  if ($_REQUEST['Picturesques']) {
    $url = "https://Picturesquedemo1.googleplex.com /_ah/api/Picturesque/v1/Picturesques";
    getUrl($url);
  }
